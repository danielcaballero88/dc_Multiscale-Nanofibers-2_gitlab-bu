...
scales = [10.*np.pi, 0.1*np.pi, 0.2*np.pi]

npuntos_pdf = 1001
thetas_pdf = np.linspace(0., np.pi, npuntos_pdf)


for nombrearchivo, scale in zip(nombresarchivos, scales):
    print "leyendo malla"
    mc = Mc.leer_de_archivo(archivo=nombrearchivo)
    print "pregraficando"
    ths, dlr, conteo, frecs, pdf = mc.get_histograma_orientaciones(nbins=10, opcion="fibras")
    print np.sum(conteo)
    ...
    nombrefigura = nombrearchivo[:-4] + "pdf.pdf"
