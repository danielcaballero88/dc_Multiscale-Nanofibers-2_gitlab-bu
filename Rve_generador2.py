...
class Malla(object):
    ...
    def trim_fibra_at_frontera(self, fib_con):
        ...
        for b in range( len(self.bordes_s.con) ): # recorro los 4 bordes
            ...
            rin = calcular_interseccion(rs0, rs1, rb0, rb1) 
            if rin is None:
                continue # con este borde no hay interseccion, paso al que sigue
            # si hubo interseccion tengo que mover el ultimo nodo y por lo tanto cambia el segmento 
            try:
                self.segs.mover_nodo(s, 1, self.nods.r, rin)
    ...
    def leer_de_archivo(cls, archivo="Malla.txt"):
        ...
        # le asigno los nodos 
        for i in range(num_r):
            malla.nods.r = coors[i] 
            malla.nods.tipos = tipos[i] 
    ...
    def graficar(self):
        ...
        self.ax.legend(loc="upper left", numpoints=1, prop={"size":4})
        plt.show()